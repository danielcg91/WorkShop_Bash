...
if [ -f secuencias1.fastq ]; then echo "El archivo existe." else echo "El archivo no existe." fi
```
...
``` {.bash code-copy="true" eval="false"}
for archivo in $file*.fastq; do if [ -f "$archivo" ]; then echo "Procesando archivo: $archivo" wc -l "$archivo" else echo "No tienes archivos fastq" fi done
```
